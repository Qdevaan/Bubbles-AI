#!/usr/bin/env python
"""Bubbles-AI one-shot launcher.

Run from anywhere::

    python server.py                 # start API + ngrok tunnel
    python server.py --no-ngrok      # API only (LAN / localhost)
    python server.py --port 9000     # override port

What it does:
  1. Boots the FastAPI app (uvicorn, factory ``bubbles.app:create_app``)
     from the ``server/`` directory so ``server/.env`` is picked up.
  2. Waits for ``/health/live`` to go green.
  3. Opens the ngrok tunnel on the project's static domain and reads the
     public URL back from ngrok's local API.
  4. Prints the URLs to connect to (LAN IP, localhost, public ngrok).
  5. Streams server + ngrok logs, prefixed, until Ctrl+C — then tears the
...
APP_FACTORY = "bubbles.app:create_app"
...
def _kill_all() -> None:
    for proc in _procs:
        ...
        try:
            if IS_WINDOWS:
                # Kill the whole tree (uv -> uvicorn -> workers).
                subprocess.run(
...
def _banner(port: int, public_url: str | None) -> None:
    ...
    print()
    print(green(bar))
    print(green(bold("  Bubbles-AI server is up")))
    print(green(bar))
    ...
    print(f"  {bold('Health')}   : {cyan(f'http://localhost:{port}/health/live')}")
...
# ── Main ────────────────────────────────────────────────────────────────────────
def main() -> int:
    parser = argparse.ArgumentParser(description="Launch the Bubbles-AI server.")
    ...
    if not SERVER_DIR.is_dir():
        print(red(f"server/ not found at {SERVER_DIR}"), file=sys.stderr)
        return 1
    if not (SERVER_DIR / ".env").is_file():
        print(red(f"missing {SERVER_DIR / '.env'} — copy .env.example and fill it"),
              file=sys.stderr)
        return 1
    if shutil.which("uv") is None:
        print(red("`uv` not on PATH — install from https://docs.astral.sh/uv/"),
              file=sys.stderr)
        return 1
