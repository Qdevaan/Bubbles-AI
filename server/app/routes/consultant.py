...
async def ask_consultant_endpoint(
    ...
    # 2. Get answer — ASYNC (no longer blocks event loop)
    ...
    # 3. Log Q&A with full metadata (async-wrapped)
    ...
    # 4. Track token usage (async-wrapped)
    await asyncio.to_thread(
...
async def ask_consultant_stream_endpoint(
    ...
    async def generate():
        ...
        try:
            # Use aclient directly — already async, no wrapping needed
        ...
        # Post-stream: log and persist with metadata (all wrapped for safety)
...
async def ask_consultant_batch(
    ...
    """Process multiple consultant questions concurrently."""
    ...
    answers = await asyncio.gather(*[_ask_one(q) for q in req.questions])
