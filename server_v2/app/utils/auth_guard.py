...
async def verify_ownership(claimed_user_id: str, verified: VerifiedUser) -> None:
    """
    Raise HTTP 403 if the verified JWT user does not own the requested resource.
    Use on DELETE endpoints and any endpoint that accesses another user's data.

    Example:
        await verify_ownership(session.user_id, user)
    """
    if verified.user_id != claimed_user_id:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Forbidden: this resource belongs to another user.",
        )
