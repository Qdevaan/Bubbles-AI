...
#### 10.1 Live Wingman Session Flow
...
SUPABASE_KEY=            # Anon key (not used by server)
...
# Optional
DEEPGRAM_KEY=            # Speech-to-text (optional)
...
if not os.path.exists(env_file):
    env_file = "../../env/.env"  # Fallback to default
load_dotenv(env_file)
```

## 3. API Contracts & Routing

### 12. Flutter Client API Contract
...
> [!NOTE]
> The `/v1/gamification/{user_id}` and `/v1/quests/{user_id}` endpoints are fully documented in [Section 14 — Gamification & Quests System](#14-gamification--quests-system).

---

### 6. Route Layer — API Endpoints

#### 6.1 `routes/health.py` — Health & Root
...
#### 6.2 `routes/sessions.py` — Session Lifecycle
...
5. `audit_svc.log()` → DB INSERT into `audit_log`
...
2. **Load contexts in parallel** (3 concurrent calls):
...
3. **Generate wingman advice** → Groq LLM call (async)
...
#### 6.3 `routes/consultant.py` — AI Consultant
...
##### `POST /v1/ask_consultant` (Blocking)
...
##### `POST /v1/ask_consultant_stream` (SSE)
...
#### 6.4 `routes/voice.py` — Voice Features
...
#### 6.5 `routes/analytics.py` — Analytics & Reports
...
##### Background: `_compute_session_analytics(session_id, user_id)`
...
#### 6.6 `routes/entities.py` — Entity Management
...
### 7.4 `utils/text_sanitizer.py` — Prompt Injection Protection
...
### 7.6 Enhanced Input Validation Constants
...
### 17.3 Prompt Injection — Additional Patterns

Extend §7.4 sanitizer with patterns seen in newer attacks:

```python
ADDITIONAL_PATTERNS = [
    r"\bdo anything now\b",           # DAN jailbreak
    r"\bact as\b.*\bmodel\b",         # Role injection
    r"\breturn.*\bjson\b.*\b{\b",     # Output format hijacking
    r"```.*system",                    # Markdown-wrapped system prompts
    r"\bprevious conversation\b",     # Context manipulation
]
```
...
### 7.1 `utils/auth_guard.py` — JWT Authentication
...
### 17.1 Ownership Verification Pattern
...
### 7.3 `utils/rate_limit.py` — SlowAPI Rate Limiter

Simple `Limiter(key_func=get_remote_address)` — keyed by client IP.

## 6. Service Specifications & Business Logic

### 5. Service Layer — Detailed Specifications

#### 5.1 `BrainService` — LLM Intelligence Layer
...
##### `get_wingman_advice(user_id, transcript, graph_context, vector_context, mode, persona) → Dict`
...
##### `ask_consultant(user_id, question, history, graph_context, vector_context, session_summaries, mode, persona) → Dict`
...
##### `extract_all_from_transcript(transcript, graph_context) → Dict`
...
- **Returns**: `{"entities": [...], "relations": [...], "events": [...], "tasks": [...], "conflicts": [...], ...metadata}`

##### `extract_highlights(transcript) → List[dict]`
...
#### 5.2 `SessionService` — Session Lifecycle
...
##### `start_session(user_id, mode, is_ephemeral, is_multiplayer, persona, device_id, session_type) → str`
...
##### `create_session_record(user_id, title, summary, mode) → str`
...
##### `log_message(session_id, role, content, speaker_label, confidence, is_ephemeral, model_used, latency_ms, tokens_used, finish_reason, turn_index) → Dict|None`
...
##### `update_session_token_usage(session_id, tokens_prompt, tokens_completion)`
...
##### `log_consultant_qa(user_id, question, answer, session_id, ...)`
...
##### `fetch_consultant_history(user_id, limit=5) → str`

- **DB READ**: `consultant_logs` — last N Q&A pairs, ordered by `created_at DESC`

##### `fetch_session_summaries(user_id, limit=3) → str`

- **DB READ**: `sessions` — completed sessions with non-null summaries

##### `count_session_turns(session_id) → int`

- **DB READ**: `session_logs` — exact count

---

#### 5.3 `EntityService` — Entity & Structured Data Persistence
...
##### `_find_fuzzy_match(user_id, canonical) → Optional[str]`
...
##### `_upsert_entity(user_id, name, entity_type, description) → Optional[str]`
...
##### `_upsert_attributes(entity_id, attributes, source_session)`
...
##### `_upsert_relation(user_id, source_id, target_id, relation, source_session)`

- **DB WRITE**: `entity_relations` — upsert on `(source_id, target_id, relation)` conflict

##### `get_entity_context(user_id, entity_id) → str`
...
##### `persist_extraction(user_id, extraction, source_session)`
...
##### `save_highlights(user_id, highlights, session_id)`
...
#### 5.4 `GraphService` — Knowledge Graph Management
...
##### `find_context(user_id, text, top_k=5) → str`
...
##### `update_local_graph(user_id, updates)`
...
#### 5.5 `VectorService` — Embedding & Long-Term Memory
...
##### `search_memory(user_id, query) → str`
...
##### `save_memory(user_id, content, session_id, memory_type="general")`
...
#### 5.6 `AuditService` — Compliance Logging

**File**: `app/services/audit_service.py`

**Purpose**: Fire-and-forget audit trail for every significant server action.

##### `log(user_id, action, entity_type, entity_id, details, ip_address, user_agent)`
...
#### 14.2 Database Tables — 6 New Tables

##### `user_gamification` — Per-User Profile (1 row per user)
...
##### `xp_transactions` — XP Audit Ledger
...
##### `achievements` — Achievement Catalog (server-managed)
...
##### `quest_definitions` — Quest Catalog (server-managed)
...
#### 14.3 `GamificationService` — XP, Streaks, Quests, Achievements
...
##### `award_xp(user_id, amount, source_type, source_id=None, description=None)`
...
##### `get_or_assign_daily_quests(user_id) → List[Dict]`
...
##### `increment_quest_progress(user_id, action_type, count=1)`
...
##### `get_gamification_profile(user_id) → Dict`
...
#### 14.4 `routes/gamification.py` — Gamification Endpoints
...
#### 14.5 Integration Points — XP Trigger Map
...
#### 14.8 Seed Data — Achievements & Quests

##### Achievement Catalog (20 badges)
...
##### Daily Quest Catalog (8 templates → 3 randomly assigned per day)
...
- Mount all business routers under `/v1` prefix
...
#### 4.2 `app/config.py` — Centralized Configuration
...
#### 4.3 `app/database.py` — Supabase Client Singleton
...
### 7.2 `utils/session_store.py` — Redis/In-Memory Session State
...
### 9. Database Table Interaction Matrix
...
#### Tables NOT Touched by Server (Flutter-only via Supabase Direct)
...
### 15.2 Per-Service Error Contracts
...
### 11.6 Graceful Degradation Matrix
...
### 11.7 Timeout Configuration Matrix
...
### 7.5 `utils/logger.py` — Structured Logging
...
### 13. Technology Recommendations — Latency Reduction & FYP Optimization
...
#### 13.1 LLM Inference — The Biggest Latency Bottleneck
...
##### 🔄 Recommendation 1: Multi-Provider LLM Gateway (LiteLLM)
...
##### ⚡ Recommendation 2: Use Cerebras as Primary Wingman Provider
...
##### 🧠 Recommendation 3: Use Google Gemini Flash for Consultant
...
#### 13.2 Embedding Model — Faster Vectors

##### 🚀 Recommendation 4: Upgrade from MiniLM to e5-small + ONNX Runtime
...
##### 🌐 Recommendation 5: Use a Cloud Embedding API as Fallback
...
#### 13.3 Database Architecture — Reducing Supabase Overhead

##### 🔌 Recommendation 6: Direct `asyncpg` Connection for Hot Paths
...
##### 📦 Recommendation 7: Batch DB Operations in Wingman Pipeline
...
##### 🗄️ Recommendation 8: pgvector HNSW Index Tuning
...
#### 13.4 Caching — Avoid Redundant Work

##### 💾 Recommendation 9: In-Memory LRU Cache for Hot Data
...
##### 🔴 Recommendation 10: Upgrade Redis to Upstash (Serverless)
...
#### 13.5 Transport & Streaming Optimization

##### 📡 Recommendation 11: Optimize SSE Streaming Pipeline
...
##### 🔄 Recommendation 12: WebSocket Upgrade for Wingman
...
#### 13.6 Code-Level Optimizations

##### 🧹 Recommendation 13: Eliminate Redundant LLM Calls
...
##### ⚙️ Recommendation 14: Preload and Warm-Up Models on Startup
...
##### 🔒 Recommendation 15: Connection Pooling for HTTP Clients
...
#### 13.7 Deployment & Hosting (Free Tier)

##### 🌍 Recommendation 16: Hosting Platform Comparison
...
##### 🐳 Recommendation 17: Optimize Docker Image
...
#### 13.8 Architectural Improvements Roadmap

##### 🏗️ Recommendation 18: Speculative Execution for Wingman
...
##### 📊 Recommendation 19: Lazy Extraction Strategy
...
##### 🌐 Recommendation 20: CDN and Edge Optimization
...
#### 13.10 Recommended Implementation Order
...
### 19.2 Existing Data Compatibility
...
#### 20.4 CI/CD Pipeline (Recommended)
...
> **End of Blueprint** — This document covers every file, every DB interaction, every data flow, every API contract, technology optimization recommendations, error handling, observability, security hardening, reliability patterns, migration strategy, and testing approach for the Bubbles AI server. Use it as the single source of truth for your rewrite.
