...
- `POST /v1/save_session` ~~remains a read-only no-op~~ **persists in batch (2026-05-13)**. v2 parity restored for the Flutter offline-buffer path: accepts `{session_id?, transcript?, logs[], title?, mode?, idempotency_key?, is_ephemeral?, user_id?(legacy/ignored)}` and returns `{status, session}`. With `session_id` it ends the existing session in place; without one it creates a fresh session and persists `logs[]` (v2/Flutter shape: `speaker`+`text` → `session_logs.role`+`content`). Then runs the same `_enqueue_post_session_jobs` as `end_session` (analytics + extract_knowledge + embeddings + detect_achievements + sentiment_scan). `is_ephemeral` short-circuits with a 200 *before* any DB write. The live per-turn path (`log_turn` / wingman loop) is still preferred; `save_session` exists for clients that buffer offline and upload all at once.
...
- **Sub-hole — done 2026-05-13:** turn-level sentiment + the per-turn analytics columns. New `sentiment_scan` worker (`analytics.sentiment` chain, `wingman/`-style JSON prompt) scores user/others turns in batches and writes `session_logs.sentiment_score` / `sentiment_label`, then re-enqueues `compute_session_analytics` (distinct job id via `job_suffix`, so it isn't deduped). `upsert_session_analytics` now also writes `average_latency_ms` / `avg_advice_latency_ms` / `avg_sentiment_score` / `dominant_sentiment` (computed from `session_logs` turns by `compute_session_analytics`); `GET /v1/session_analytics/{id}` returns a real `sentiment_trend` (`analytics_repo.sentiment_trend_for_session`). `end_session` enqueues `sentiment_scan` alongside the rest. *(Still skipped: writing the prod `sentiment_logs` table — its botched dual-column shape (`sentiment_score`+`score`, `emotion_label`+`label`) isn't worth replicating; the canonical data lives on `session_logs`.)* Tests: `tests/unit/test_extraction.py` (`score_turn_sentiments`), `tests/integration/test_job_sentiment_scan.py`.
...
- **Follow-ups landed 2026-05-13:** rolling-summary-every-20-turns and multiplayer turn handling. (1) New `workers/jobs/rolling_summarize.py` ARQ job: pulls the last 40 `session_logs` turns, generates a short summary via `generate_summary`, appends `\\n---\\n[Turn N] {summary}` to `sessions.summary`. Wired from `process_transcript_wingman._persist_advice_and_followups` — fires every `_ROLLING_SUMMARY_EVERY_N_TURNS=20` LLM turns; ARQ-deduped by `(session_id, turn_index)`. (2) `StartSessionRequest.is_multiplayer` + `sessions_repo.start(is_multiplayer=)` + `SessionOut.is_multiplayer` — v2 parity (just persisted; no downstream branching).
...
- Quest **mission** endpoints — `POST /v1/quests/{user_id}/{user_quest_id}/answer` (question_set: records one answer keyed by `question_id` in `user_quests.brief_state`; progress = #distinct questions answered; completes + awards `xp_reward` at `target`; rejects unknown ids when `brief.questions[].id` is set) and `POST /v1/quests/{user_id}/{user_quest_id}/attach_session` (conversation: completes + awards XP iff the attached session has ≥ `brief.min_turns` user turns; records the attempt in `brief_state` either way). New repo: `get_user_quest`, `get_quest_def`, `record_question_answer`, `complete_conversation_quest`, `award_quest_xp_once`, `bump_quest_progress_by_action`. Schemas `QuestAnswerRequest` / `QuestAttachSessionRequest` / `QuestMissionResult`. **LLM judge landed 2026-05-13:** when `brief.criteria` is set, `attach_session` also runs `analytics.mission_eval` (`evaluate_conversation_mission` → `(passed, reason)`); completion now requires both the turn-count gate and `eval_passed is not False`. Eval result is stored in `brief_state.last_attach.{eval_passed, eval_reason}`. A failed/unparseable judge returns `(None, None)` → falls back to the turn-count gate alone, so an LLM hiccup never falsely fails the user.
- Live-loop XP wiring — `process_transcript_wingman`'s background path now awards a small per-turn XP (`wingman_turn`, daily-capped, idempotent per turn) and calls `bump_quest_progress_by_action(user_id, "use_wingman_turns")`.
- **Consultant quest hooks — done 2026-05-13:** `ask_consultant` / `ask` / `ask_consultant/batch` now nudge the `ask_consultant` and `save_memory` daily quests (`_maybe_bump_quests` → `bump_quest_progress_by_action`, batch passes `delta=len(questions)`). Best-effort like the enqueue paths — fires only when `app.state.db` is up, swallows errors, never turns an answer into a 503; runs on `app.state.bg` if present, else inline (two quick `UPDATE`s). Tests: `tests/unit/test_routes_consultant.py` (`test_consultant_bumps_quests_when_pool_present`, `test_consultant_skips_quest_bump_when_no_pool`).
...
2. ~~**H2 + H3 — per-turn store + `process_transcript_wingman`.**~~ **Done 2026-05-12.** `session_logs` (Alembic `0004`), `db/repo/session_logs.py`, `POST /v1/log_turn`, `GET /v1/session_replay/{id}`, `POST /v1/process_transcript_wingman`, `end_session` assembles from rows. Spec: `docs/superpowers/specs/2026-05-12-v5-port-h2-h3-turn-store-wingman-design.md`. Follow-ups landed 2026-05-13: per-turn analytics columns + `sentiment_scan` worker (see §5 H2). Still: rolling-summary; multiplayer turns; the prod `sentiment_logs` table.
...
8. **All findings closed.** Left: `git rm -r legacy/server_v2/` once nothing references it. *All* doc-noted follow-ups closed 2026-05-13: rolling-summary every-N-turns ✅, multiplayer turns ✅, LLM evaluator for conversation missions ✅, `ask_consultant`/memory-save quest hooks ✅. Only deliberate skip: prod `sentiment_logs` table (botched dual-column shape; canonical data lives on `session_logs`).
