...
8. **All findings closed.** Left: `git rm -r legacy/server_v2/` once nothing references it. Remaining minor doc-noted follow-ups: rolling-summary every-N-turns; multiplayer turns; prod `sentiment_logs` table; LLM evaluator for conversation missions; `ask_consultant`/memory-save quest hooks. None blocking.
